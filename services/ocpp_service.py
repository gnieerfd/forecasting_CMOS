...
Requires: ocpp (from PyPI), websockets>=11
...
if _OCPP_AVAILABLE:
    ...
    async def _on_connect(websocket):
        ...
        # Menyesuaikan aturan websockets v16.0 (parameter path dihilangkan)
        try:
            path = websocket.request.path
        except AttributeError:
            path = websocket.path # Fallback untuk versi lama
    ...
    async def run_server(host: str = "0.0.0.0", port: int = 9000) -> None:
        ...
        await asyncio.Future()  # Mengunci agar server berjalan selamanya
